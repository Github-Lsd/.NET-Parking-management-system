    ...
    public partial class SearchParking : Form
    {
        ...
        private void Del_Click(object sender, EventArgs e)
        {
            foreach(DataGridViewRow row in dataGridView1.SelectedRows)
            {
                ...
                {
                    result = MessageBox.Show(id+"号停车位正在使用中，确定要删除吗？","警告",MessageBoxButtons.OKCancel,MessageBoxIcon.Question);
                    
                }
                else
                {
                    result = MessageBox.Show("确定要删除" + id + "号停车位吗？", "警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                }
