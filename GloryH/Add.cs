...
namespace GloryH
{
    ...
    public partial class Add : Form
    {
        ...
        public void AddParking()
        {
            ...
            p.Car_user = card_value1.Text;
            p.Car_number = car_number1.Text;
            p.State = state;
            p.Begin_time = Convert.ToDateTime(begin_time);

            /*string sql = "insert into parking(parking_id,card_value,car_user,car_number,state,begin_time) values("+p.Parking_id+",'"+p.Card_value+"','"+p.Car_user+"','"+p.Car_number+"',"+p.State+",'"+ p.Begin_time + "')";
            dBHelper.ADD(sql);*/
            p.insert();
